...
- **主要子模块**：
  ...
  - `scripts/`：包含环境初始化脚本，如 `use-rust-env.sh`。
...
桌面端应用基于 Tauri v2 构建，这要求本地安装有 Rust 工具链。LightFlux 特别提供了一个脚本 `scripts/use-rust-env.sh`，用于在特定目录下隔离开发环境，这对于保持系统整洁非常有用。

根据 `lightflux/src-tauri/Cargo.toml` 的定义，Rust 版本应至少为 **1.77.2**。

**使用 `use-rust-env.sh` 配置 Rust**：
该脚本通过设置 `RUSTUP_HOME` 和 `CARGO_HOME` 环境变量，将 Rust 工具链安装在非标准目录（默认为 `$HOME/Desktop/Dev_env`）。

```bash
# 查看脚本逻辑
cat scripts/use-rust-env.sh
```

```mermaid
flowchart TD
    Start[运行 scripts/use-rust-env.sh] --> CheckEnv{是否定义 LIGHTFLUX_DEV_ENV?}
    CheckEnv -- 是 --> SetPaths[使用自定义路径]
    CheckEnv -- 否 --> SetDefault[使用默认路径: ~/Desktop/Dev_env]
    SetPaths --> ExportVars[导出 RUSTUP_HOME / CARGO_HOME]
    SetDefault --> ExportVars
    ExportVars --> UpdatePath[更新 PATH 包含 $CARGO_HOME/bin]
    UpdatePath --> Exec[执行后续命令或显示版本]
```
...
```
这对于需要在同一台机器上维护多个 Rust 版本的开发者来说非常有用。如果直接执行该脚本而不带参数，它将打印当前的 Rust 版本信息。
...
- 如果使用了 `use-rust-env.sh`，请确保在当前终端会话中 `source` 了该脚本或通过该脚本运行命令。
...
flowchart TD
    ...
    Type -- Rust相关 --> RustCheck[检查 scripts/use-rust-env.sh 是否生效]
...
- [scripts/use-rust-env.sh](scripts/use-rust-env.sh)：Rust 环境隔离脚本。
