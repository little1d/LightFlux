...
| FEAT-003 | feature | User: delete release and CLI remote repos | Required content/releases are preserved or migrated, then both exact GitHub repos are deleted and confirmed absent | GitHub | high | ready |
...
- Verification: Gitleaks scanned 112 commits and found only the documented
...
| Gitleaks Git history scan | pass | 112 commits | One test-fixture false positive; no real credential. |
...
| `cd server && npm test` | pending | Server | |
| `cargo check --manifest-path lightflux/src-tauri/Cargo.toml` | pending | Desktop | |
| repository secret/history audit | pending | Open-source readiness | |
| GitHub auxiliary repository deletion | pending | Remote state | |
